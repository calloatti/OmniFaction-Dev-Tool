...
namespace Calloatti.OmniFaction
{
  ...
  public class OmniFactionService : ILoadableSingleton, IDisposable
  {
    ...
    // Entity faction cache (keyed by Unity entity ID)
    private static readonly Dictionary<EntityId, string> _entityFactionCache = new Dictionary<EntityId, string>();
    ...
    public static void RemoveFactionForEntity(GameObject entity)
    {
      ...
      _entityFactionCache.Remove(entity.GetEntityId());
    }

    public static string GetCachedFaction(GameObject entity)
    {
      if (entity == null) return null;
      _entityFactionCache.TryGetValue(entity.GetEntityId(), out string faction);
      return faction;
    }
    ...
    private void GiveStartingInventory(BlockObject blockObject, GameModeSpec gameMode)
    {
      ...
      simpleOutputInventory.Inventory.GiveExistingIgnoringCapacity(new GoodAmount("Berries", gameMode.StartingFood));
      simpleOutputInventory.Inventory.GiveExistingIgnoringCapacity(new GoodAmount("Water", gameMode.StartingWater));
    }
